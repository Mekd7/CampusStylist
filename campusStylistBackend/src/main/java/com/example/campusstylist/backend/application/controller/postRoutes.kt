...
fun Route.postRoutes(postService: PostService, userService: UserService) {
    authenticate("auth-jwt") {
        post("/posts") {
            ...
            val user = userService.findByEmail(email) ?: return@post call.respond(HttpStatusCode.Unauthorized)
        ...
        put("/posts/{id}") {
            ...
            val user = userService.findByEmail(email) ?: return@put call.respond(HttpStatusCode.Unauthorized)
        ...
        delete("/posts/{id}") {
            ...
            val user = userService.findByEmail(email) ?: return@delete call.respond(HttpStatusCode.Unauthorized)
